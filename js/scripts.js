...
$(function(){
	...
	function initPeppermint() {
		...
		function elasticShit(cur, next) {
			console.log(cur+ ' '+next);
			if (cur === next) {return;}
			var dir = '', timer = 200;
		  cur < next ? dir = 'right' : dir = 'left';
			$('.peppermint-dots li:eq('+cur+')').append('<div class="elast"></div>');
			paper1.clear();
			paper2.clear();
			paper3.clear();
			
			var needed = paper1;
			if (next == 1) {needed = paper2}
			else if (next == 2) {needed = paper3}
			
      needed.customAttributes.arc = function (xloc, yloc, value, total, R) {
			...
			if (dir === 'right') {
				var distance = $('.peppermint-dots li:eq('+next+')').offset().left - $('.peppermint-dots li:eq('+cur+') .elast').offset().left;
				$('.peppermint-dots li:eq('+cur+') .elast').animate({'right':'-'+distance+'px'}, timer);
				$('.peppermint-dots li:eq('+cur+') .elast').delay(timer/3).animate({'left':distance+'px'}, timer, function(){
					$('.peppermint-dots .elast').remove();
				});
			}
			else {
				var distance = $('.peppermint-dots li:eq('+cur+') .elast').offset().left - $('.peppermint-dots li:eq('+next+')').offset().left - 10;
				$('.peppermint-dots li:eq('+cur+') .elast').css({'right':'2px'});
				$('.peppermint-dots li:eq('+cur+') .elast').animate({'left':'-'+distance+'px'}, timer);
				$('.peppermint-dots li:eq('+cur+') .elast').delay(timer/3).animate({'right':distance+'px'}, timer, function(){
					$('.peppermint-dots .elast').remove();
				});
			}
		};
		...
		slider1.Peppermint({
		  ...
		  dots: true,
		  onSlideChange: function(num) {
			  var next = num;
			  elasticShit(curSlideNumber, next);
			  curSlideNumber = next;
		  }
		});
		...
		if (slider1) {
			var paper1 = Raphael($('.peppermint-dots li:eq(0)')[0], 16, 16);
	    var paper2 = Raphael($('.peppermint-dots li:eq(1)')[0], 16, 16);
	    var paper3 = Raphael($('.peppermint-dots li:eq(2)')[0], 16, 16);
	    
	    
	    paper1.customAttributes.arc = function (xloc, yloc, value, total, R) {
				var alpha = 360 / total * value,
				a = (90 - alpha) * Math.PI / 180,
				x = xloc + R * Math.cos(a),
				y = yloc - R * Math.sin(a),
		...
		$('.partners-arrow--right').click(slider2.data('Peppermint').next);
		$('.partners-arrow--left').click(slider2.data('Peppermint').prev);
	...
	function slider3() {
		var slider3 = $('.other .peppermint');
		slider3.Peppermint({
		  slideshow: false,
		  dots: true
		});
		
		var slider4 = $('.clnts .peppermint');
		slider4.Peppermint({
		  slideshow: false,
		  dots: true
		});
...
// equal heights
	...
	function submenuWidth() {
		...
		$l.each(function(){
			var $this = $(this);
			$this.css({'padding':'0', 'margin':'0', 'width':'auto'});
			w += $this.width();
		});
		
		var bw = ($m.width() - w) / (($l.length-1)*2);
		$l.css({'margin-left':(bw-2)+'px', 'margin-right': (bw-2)+'px'});
		$('.catmenu-link:first').css({'margin-left':'0'});
		$('.catmenu-link:last').css({'margin-right':'0'});
	};
	
	submenuWidth();
	$(window).resize(function(){submenuWidth()});
